...
exports.messageSize = exports.amqpEncode = void 0;
...
const PropertySizeDescription = 3 + // sizeOf DescribedFormatCode.Size (3 byte)
    1 + // sizeOf FormatCode.List32 (byte)
    4 + // sizeOf field numbers (uint)
    4; // sizeof propertySize (uint)
function amqpEncode(writer, { content, messageProperties, applicationProperties, messageAnnotations }) {
...
exports.amqpEncode = amqpEncode;
function messageSize({ content, messageProperties, applicationProperties, messageAnnotations }) {
    return (lengthOfContent(content) +
...
function lengthOfProperties(properties) {
    if (!properties)
        return 0;
    return PropertySizeDescription + getPropertySize(properties);
}
function lengthOfApplicationProperties(applicationProperties) {
    if (!applicationProperties.length) {
        return 0;
    }
    return PropertySizeDescription + getListSize(applicationProperties);
}
function lengthOfMessageAnnotations(messageAnnotations) {
    if (!messageAnnotations.length) {
        return 0;
    }
    return PropertySizeDescription + getListSize(messageAnnotations);
}
function writeApplicationProperties(writer, applicationPropertiesList) {
...
function writeMessageAnnotations(writer, messageAnnotationsList) {
    ...
    messageAnnotationsList
        ...
        .forEach((elem) => {
        amqpWriteString(writer, elem.key);
        if (elem.value instanceof publisher_1.AmqpByte) {
...
function writeProperties(writer, properties) {
    if (!properties) {
        return;
    }
    // write applicationData header
...
function writeContent(writer, content) {
    // write applicationData header
    writer.writeByte(FormatCode.Described);
    writer.writeByte(FormatCode.SmallUlong);
...
function getPropertySize(properties) {
    return (getSizeOf(properties.messageId) +
...
function amqpWriteString(writer, data) {
    if (!data) {
...
function amqpWriteSymbol(writer, data) {
    if (!data) {
        return amqpWriteNull(writer);
    }
    const content = Buffer.from(data);
...
function amqpWriteNumber(writer, data) {
    if (!data) {
        return amqpWriteNull(writer);
    }
...
function amqpWriteIntNumber(writer, data) {
    if (!data) {
        return amqpWriteNull(writer);
    }
...
function amqpWriteByte(writer, data) {
    writer.writeByte(FormatCode.Ubyte);
    writer.writeByte(data.byteValue);
}
function amqpWriteBuffer(writer, data) {
    if (!data || !data.length) {
        return amqpWriteNull(writer);
    }
    if (data.length < 256) {
...
function amqpWriteNull(writer) {
    writer.writeByte(FormatCode.Null);
}
function amqpWriteDate(writer, date) {
    if (!date) {
        return amqpWriteNull(writer);
    }
...
function getSizeOf(value) {
    if (!value) {
        return 1;
    }
    if (value instanceof publisher_1.AmqpByte) {
        return 1 + 1;
    }
    ...
    throw new Error(`Unsupported type: ${(0, node_util_1.inspect)(value)}`);
}
function toList(applicationProperties) {
...
function toAnnotationsList(annotations) {
    if (!annotations)
        return [];
